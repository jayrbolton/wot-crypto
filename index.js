...
const debug = require('debug')('wot-crypto')
...
crypto.hashPass = function hashPass (pass, salt, callback) {
  debug('hashing passhprase to generate a secret key and salt')
  assert(pass && pass.length > 0 && typeof pass === 'string', 'pass in a string password')
  assert(typeof pass === 'string' || pass instanceof Buffer, 'passphrase should be a string or buffer')
...
crypto.encrypt = function encrypt (key, message) {
  debug('encrypting message with key length ' + key.length + ' and message length ' + message.length)
  assert(key && key.length > 0, 'pass in a secret key')
...
crypto.decrypt = function decrypt (encrypted, key) {
  debug('decrypting encrypted message with secret key (crypto_secretbox)')
...
crypto.createSignKeypair = function signKeyPair (seed) {
  debug('creating public and secret key pair for signing')
...
crypto.createBoxKeypair = function boxKeyPair (seed) {
  debug('creating public and secret key pair for encryption')
  assert(seed && seed.length, 'pass in a seed buffer of length 32')
  seed = Buffer.from(seed)
...
crypto.sign = function sign (message, sk) {
  debug('signing message with a crypto_sign secret key')
  message = Buffer.from(message)
...
crypto.openSigned = function openSigned (signed, pk) {
  debug("verifying and opening a signed message using the signer's pubic key")
...
crypto.sendEncrypted = function sendEncrypted (message, pk, sk) {
  debug("encrypting a message with a receiver's public key")
...
crypto.openEncrypted = function openEncrypted (encrypted, pk, sk) {
  debug('opening an encrypted message using public and secret keys')
